...
| `tail-logs`              | Use for a **snapshot of the newest** log lines near one instant. Parameter `startNanosInclusive` is the **range end** (as-of time, nanoseconds); the server calls **`query_range`** with a **1h lookback** ending at that instant (minus optional `delayForSeconds` for ingest lag), **backward** direction, plus `limit`. Loki does not support log stream selectors on instant `/query`. Not live WebSocket tailing. | Loki `GET /loki/api/v1/query_range`                        |
